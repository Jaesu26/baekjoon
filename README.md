...
`9.` 세그먼트 트리 구축은 $O(N)$에 수행할 수 있다 (왜 $O(N \log N)$이라 생각했지...)

`10.` `del` 내장 함수로 리스트 슬라이싱 객체를 제거하는 건 개별 원소 각각을 삭제하는 것에 비해 매우 빠르게 동작한다

`11.` 백트래킹으로 스도쿠를 채울 때 다음 수를 정적으로 순회하는 대신 shuffle 한 뒤 순회하면 극단적인 테케를 피할 수 있어 평균 실행 시간을 크게 단축시킬 수 있다 (실제로 이를 추론해내어 TLE 구현에 무작위화만 추가하여 [스토쿠](https://jungol.co.kr/problem/1824) 문제를 맞혔다)
